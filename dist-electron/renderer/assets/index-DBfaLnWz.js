...
function useEvents() {
  ...
  reactExports.useEffect(() => {
    const cleanup = window.electronAPI.onSystemEvent((event) => {
      setLastEvent(event);
    });
...
function useIpc() {
  const getSessionInfo = reactExports.useCallback(async () => {
    return getApi().getSessionInfo();
  }, []);
  const getUnitInfo = reactExports.useCallback(async (unitId) => {
    return getApi().getUnitInfo(unitId);
  }, []);
  const getUnitMessages = reactExports.useCallback(async (unitId, opts) => {
    return getApi().getUnitMessages(unitId, opts);
  }, []);
  const sendMessage = reactExports.useCallback(async (content2) => {
    const result = await getApi().sendMessage(content2);
    return result.ok;
  }, []);
  const terminateSession = reactExports.useCallback(async () => {
    const result = await getApi().terminateSession();
    return result.ok;
  }, []);
  const getFsTree = reactExports.useCallback(async (mode) => {
    return getApi().getFsTree(mode);
  }, []);
  const readFile = reactExports.useCallback(async (path2) => {
    return getApi().readFile(path2);
  }, []);
  const getProviders = reactExports.useCallback(async () => {
    return getApi().getProviders();
  }, []);
  const getModels = reactExports.useCallback(async (provider) => {
    return getApi().getModels(provider);
  }, []);
  const validateConfig = reactExports.useCallback(async (config) => {
    return getApi().validateConfig(config);
  }, []);
  const startSession = reactExports.useCallback(async (config) => {
    return getApi().startSession(config);
  }, []);
  const loadPersistedConfig = reactExports.useCallback(async () => {
    return getApi().loadPersistedConfig();
  }, []);
  const savePersistedConfig = reactExports.useCallback(async (config) => {
...
function ThreeColumnLayout({ children }) {
  ...
  const onMouseDown = reactExports.useCallback((handle2, e) => {
    e.preventDefault();
    dragging.current = handle2;
...
function TreeNode({ node: node2, selectedUnitId, onSelectUnit, depth }) {
  const [expanded, setExpanded] = reactExports.useState(true);
  const hasChildren = node2.children.length > 0;
  const isSelected = node2.unitId === selectedUnitId;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
    ...
    expanded && hasChildren && node2.children.map((child) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      TreeNode,
      {
        node: child,
        selectedUnitId,
...
function isFilePath(text2) {
  return text2.startsWith("/") && text2.includes("/") && hasFileExtension(text2);
}
...
function stripLineRange(path2) {
  const m = path2.match(/^(.+):(\d+(?:-\d+)?)$/);
...
const BOLD_RE = /\*\*(.+?)\*\*/g;
function parseSegments(text2) {
  const segments = [];
  const parts = [];
  let lastIdx = 0;
  for (const m of text2.matchAll(BACKTICK_RE)) {
    if (m.index > lastIdx) {
      parts.push({ text: text2.slice(lastIdx, m.index), isCode: false });
    }
    parts.push({ text: m[2], isCode: true });
    lastIdx = m.index + m[0].length;
  }
  if (lastIdx < text2.length) {
    parts.push({ text: text2.slice(lastIdx), isCode: false });
  }
...
function renderInlineContent(text2, options) {
  const segments = parseSegments(text2);
  const nodes = [];
  let keyIdx = 0;
  for (const seg of segments) {
    ...
    switch (seg.type) {
      ...
      case "code":
        nodes.push(
          /* @__PURE__ */ jsxRuntimeExports.jsx("code", { className: "inline-code", children: seg.value }, key)
        );
        break;
      case "bold":
        nodes.push(
          /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: seg.value }, key)
        );
        break;
      case "file-path": {
        const name2 = pathFileName(seg.fullPath);
        ...
        const startLine = seg.lineRange ? parseInt(seg.lineRange.split("-")[0], 10) : void 0;
        ...
        if (opener) {
          nodes.push(
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                ...
                onClick: (e) => {
                  e.stopPropagation();
                  opener(seg.fullPath, name2, startLine);
                },
                children: [
                  "@",
                  displayText
          ...
          nodes.push(
            /* @__PURE__ */ jsxRuntimeExports.jsxs("code", { className: "inline-code", title: titleText, children: [
              "@",
              displayText
            ] }, key)
          );
...
function buildBreadcrumb(sessionInfo, unitId) {
  ...
  const crumbs = [];
  function findPath(node2, target, path2) {
    path2.push({ unitId: node2.unitId, label: `${node2.level}: ${node2.unitId}` });
    if (node2.unitId === target) return true;
    for (const child of node2.children) {
      if (findPath(child, target, path2)) return true;
    }
    path2.pop();
    return false;
  }
...
function ChatPanel({ unitId, sessionInfo, messages, onSendMessage, onSelectUnit, onOpenFile, refs, onRemoveRef }) {
  ...
  const handleSend = reactExports.useCallback(() => {
    ...
    if (!text2 && refs.length === 0) return;
    let content2 = text2;
    if (refs.length > 0) {
      const refLine = refs.map(formatRefForMessage).join(" ");
      content2 = content2 ? `${content2} ${refLine}` : refLine;
    }
    onSendMessage(content2);
    setInput("");
...
function ExternalAnchor({ href, children, ...rest }) {
...
function sourceLineProps(node2) {
...
function buildMdComponents() {
  ...
  for (const tag of LEAF_BLOCK_TYPES) {
    components[tag] = ({ node: node2, children, ...rest }) => {
      const extra = sourceLineProps(node2);
      return React.createElement(tag, { ...rest, ...extra }, children);
    };
  }
  components.code = ({ node: node2, inline, className, children, ...rest }) => {
    if (inline) {
...
function OnboardingPage({ onComplete, initialConfig, error }) {
  const [provider, setProvider] = reactExports.useState(initialConfig?.provider ?? "anthropic");
  const [modelName, setModelName] = reactExports.useState(initialConfig?.modelName ?? "claude-sonnet-4-20250514");
  const [apiKey, setApiKey] = reactExports.useState("");
  const [baseUrl, setBaseUrl] = reactExports.useState(initialConfig?.baseUrl ?? "");
  const [projectRoot, setProjectRoot] = reactExports.useState(initialConfig?.projectRoot ?? "");
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col h-screen bg-[var(--color-bg)]", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "titlebar-drag" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-center justify-center flex-1", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-full max-w-md bg-white rounded-xl shadow-sm border border-stone-200 p-8", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-2xl font-bold text-gray-800 mb-1", children: "Welcome to Elenchus" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm text-gray-500 mb-6", children: "Configure your LLM provider to get started." }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: "LLM Provider" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "select",
        {
          ...
          children: KNOWN_PROVIDERS.map((p) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: p.id, children: p.name }, p.id))
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: "Model" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "select",
        {
          className: "w-full border border-stone-200 rounded-lg px-3 py-2 text-sm mb-4 focus:outline-none focus:border-stone-400",
          ...
          children: [
            models.map((m) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: m.id, children: m.name }, m.id)),
            models.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: modelName, children: modelName })
          ]
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: "API Key" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          type: "password",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: [
        "Base URL ",
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-gray-400 font-normal", children: "(optional, for third-party providers)" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "block text-sm font-medium text-gray-700 mb-1", children: [
        "Project Directory ",
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-gray-400 font-normal", children: "(agent working directory)" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
      ...
      (error || localError) && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm text-red-600 bg-red-50 rounded-lg px-3 py-2 mb-4", children: error ?? localError }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
...
function App() {
  ...
  const handleRemoveRef = reactExports.useCallback((index2) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col h-full", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 min-h-0", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(ThreeColumnLayout, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col h-full", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-1/2 min-h-0 overflow-y-auto", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          WorkspaceDir,
          {
            tree: fsTree,
            mode: fsMode,
...
class ErrorBoundary extends reactExports.Component {
  ...
  render() {
    if (this.state.error) {
      return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { padding: 20, fontFamily: "monospace", whiteSpace: "pre-wrap", color: "red" }, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Renderer Error" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { children: this.state.error })
      ] });
    }
